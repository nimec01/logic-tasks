...
import Data.List(transpose, nub, delete, intersperse,null,maximum)
...
generSynTree (minnode, maxnode) maxdepth lits 
  | minnode == 0 && maxnode > 2= do
    e <-elements [True,False] if e then  genSynTreenosub lits  else do
  ...
  | otherwise = do
    elements [genSynTreewithonesub (minnode, maxnode) maxdepth lits ,genSynTreewithtwosub (minnode, maxnode) maxdepth lits]
