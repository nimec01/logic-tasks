...
genMinInst MinMaxConfig {cnfConf = NormalFormConfig {baseConf = BaseConfig{..},..},..} = do
   ...
   where
     getDnf = genDnf (minClauseAmount, maxClauseAmount) (minClauseLength, maxClauseLength) usedLiterals True
...
description MinInst{..} = do
  ...
  paragraph $ indent $ do
    translate $ do
      let formulaStr = show $ mkDnf [mkCon [Pos 'A', Neg 'B'], mkCon [Neg 'C', Neg 'D']]
