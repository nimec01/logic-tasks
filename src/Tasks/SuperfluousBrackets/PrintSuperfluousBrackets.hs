...
allocateBracketToSubtree :: SynTree (BinOp, Integer) Char -> SynTree (BinOp, Integer) Char -> (BinOp, Integer) -> Integer -> Bool -> Maybe BinOp -> Maybe Integer -> Gen String
...
nonRootDisplay synTree@(Not a) brackets _ serial = do
    ...
    then do
        ...
        return (showOperatorNot ++ formula)
nonRootDisplay (Binary operWithSerial a b) brackets fatherOperator serial = allocateBracketToSubtree a b operWithSerial brackets True fatherOperator serial
