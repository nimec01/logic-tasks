...
checkLegalCNFConfig LegalCNFConfig{cnfConfig = CnfConfig {baseConf = BaseConfig{..}, ..}, ..}
    ...
    | fromIntegral maxClauseAmount > min 15 (product (take maxClauseLength (reverse [1 .. (2 * fromIntegral (length usedLiterals) ::Integer )]))) || (externalGenFormulas > 0 && fromIntegral maxClauseAmount > min 15 (product (take maxClauseLength (reverse [1 .. fromIntegral (length usedLiterals) ::Integer]))))
