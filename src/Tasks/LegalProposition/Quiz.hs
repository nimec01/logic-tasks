...
generateLegalPropositionInst LegalPropositionConfig  {formulaConfig = SynTreeConfig {..}, ..} = do
    treeList <- vectorOf (fromIntegral formulasNum) (genSynTree (minNodes, maxNodes) maxDepth usedLiterals atLeastOccurring useImplEqui)
    serialNumOfWrong <- vectorOf (fromIntegral illegalNum) (choose (1, fromIntegral formulasNum) )`suchThat` (\list -> nubOrd list ==list)
