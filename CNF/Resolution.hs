...
genRes (minLen,maxLen) steps lits = do
  ...
  return (map Clause (nub (map sort shuffled)))
 ...
 where buildClauses xs ys zs
        ...
        | otherwise =       do case ys of []     -> do chosenChar <- elements xs
                                          ...
                                          _      -> do let underMin = filter (\clause -> length clause < minLen) ys
                                                       ...
                                                       choice <- if length chosenClause == 1 || chosenClause `elem` underMin 
                                                                      then return 1 
                                                                      else if length chosenClause == maxLen then return 2 else chooseInt (1,2)
                                                       chosenChar <- elements chooseableLits
                                                       if choice == 1 then do let newClause1 = Literal chosenChar : chosenClause
                                                                              let newClause2 = Not chosenChar : chosenClause
                                                                              let newSet = newClause1 : newClause2 : delete chosenClause ys
                                                                              ...
                                                                              if any (\cl -> getLs cl `elem` zs) possible
                                                                                ...
                                                                                else buildClauses xs (nub (map sort newSet)) (newClause1 : newClause2 : zs)
